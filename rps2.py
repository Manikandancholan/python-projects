...
def save_score(user_score, computer_score, win_status):
    ...
    try:
        query = """
        insert into user_scores (username, user_score, cpu_score, win_loss_status) values (%s, %s, %s, %s)
        """
        cursor.execute(query, (username, user_score, computer_score, win_status))
        connection.commit()
        connection.close()
    except Exception as err:
        print(err)
